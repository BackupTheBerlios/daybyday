	...
	public static void createWindow(final JFrame frame, Object[] obj) {
		{
			...
			ok.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					SubjectDto obj = (SubjectDto) subjectCombo
							.getSelectedItem();
					TeacherDto obj2 = (TeacherDto) respCombo.getSelectedItem();
					EquipmentDto obj3 = (EquipmentDto) materialCombo
							.getSelectedItem();
					...
					try {
						...
						int endhour = Integer.parseInt(firstEndTextField
								.getText());
						int endmin = Integer
								.parseInt(endEndTextField.getText());

						int bgday = Integer.parseInt(firstTextFieldp.getText());
						int bgmonth = Integer.parseInt(fTextFieldp.getText());
						int endday = Integer.parseInt(firstEndTextFieldp
								.getText());
						int endmonth = Integer.parseInt(endEndTextFieldp
								.getText());
						...
						Timestamp startDate = toTimeStamp(2005, bgmonth - 1,
								bgday, bghour, bgmin, 00);
						Timestamp endDate = toTimeStamp(2005, endmonth - 1,
								endday, endhour, endmin, 00);
						...
						CourseDto newdto = new CourseDto(obj.getName(),
								sectionname, formation.getName(), formation
										.getYear(), "cours", startDate,
								endDate, "groupe1", obj2.getName(), obj2
										.getFirstname(), obj4.getName(), obj4
										.getBuilding(), obj4.getArea(), null,
								null, null, infoList.getText(), new Integer(
										colorf), new Boolean(true));

						MainFrame.myDaybyday.createCourse(newdto);
						if (section instanceof Section) {
							MainFrame.myDaybyday
									.updateSection(((Section) section).getDTO());

							((Section) section).upDateDto(MainFrame.myDaybyday
									.getSection(((Section) section).getDTO()
											.getSectionPK()));
							framefinal.dispose();
							df.changeSource(new Section(((Section) section)
									.getDTO(), section.getFather()));
						}
						if (section instanceof Formation) {
							try {
								MainFrame.myDaybyday
										.updateFormation((FormationDto) ((Formation) section)
												.getDto());
						...
						TeacherDto resp = MainFrame.myDaybyday
								.getTeacher(new TeacherPK(obj2.getName(), obj2
										.getFirstname()));
						MainFrame.myDaybyday.updateTeacher(resp);

					} catch (java.lang.NumberFormatException e) {
						mainframe.showError(frame,
								"Champs manquant(s) ou mal renseigné(s)");
						...
						mainframe.showError(frame,
								"Ressource(s) indisponible(s)");
					} catch (CourseConfusionException e) {
						mainframe
								.showError(
										frame,
										"Attention! recouvrement du cours que vous voulez créer avec un cours existant!");
