 ...
 * a new course.
 */
public class WindowModifyCourse extends WindowAbstract {
	...
	{		
		...
		Course cours =  (Course) obj[13];
		final CourseDto oldcourse = (CourseDto) cours.getDto();
		...
		final Integer bgHour   = new Integer(Grid.gridBgHour + cours.getBgHour());//(Integer) obj[8];
		final Integer bgMinute  = new Integer(cours.getBgMin() * (60 / Grid.gridSlice));//(Integer) obj[9];
		final int index  = ((Integer) obj[10]).intValue();
		final Integer endHour   = new Integer(Grid.gridBgHour + cours.getEndHour());//(Integer) obj[11];
		final Integer endMinute  = new Integer(cours.getEndMin() * (60 / Grid.gridSlice) );//(Integer) obj[12];
		...
		JPanel totalePanel = new JPanel(null);
			...
			SubjectDto oldSubject =  MainFrame.myDaybyday.getSubject(oldcourse.getSubjectId());
			Object[] subjectList =  (Object[]) obj[1];
			int subjectindex = 0;
			for (int i = 0; i < subjectList.length; ++i){
				if (oldSubject.getSubjectId().equalsIgnoreCase(((SubjectDto)subjectList[i]).getSubjectId()) == true){
					subjectindex = i; subjectCombo.setSelectedIndex(i);
				}
			}
		...
		subjectCombo.setSelectedIndex(subjectindex);
		...
		JPanel totalePanel2 = new JPanel(null);
			...
			if (teacherList.size()>0){
				...
				for (int i = 0; i < respList.length; ++i){
					if (oldResp.getTeacherId().equalsIgnoreCase(((TeacherDto)respList[i]).getTeacherId()) == true){
						respindex = i; respCombo.setSelectedIndex(i);
					}
				}
			}
			String aucun= "aucun";
			...
			ArrayList roomList = MainFrame.myDaybyday.getRoomsOfCourse(oldcourse.getCourseId());
			if (roomList.size()>0){
				RoomDto oldResp =  (RoomDto)roomList.get(0);
				Object[] respList =  (Object[]) obj[3];
				int roomindex = 0;
				for (int i = 0; i < respList.length; ++i){
					if (oldResp.getRoomId().equalsIgnoreCase(((RoomDto)respList[i]).getRoomId()) == true){
						roomindex = i; roomCombo.setSelectedIndex(i);
					}
				}
			}
			...
			ArrayList materialList = MainFrame.myDaybyday.getEquipmentsOfCourse(oldcourse.getCourseId());
			if (materialList.size()>0){
				EquipmentDto oldResp =  (EquipmentDto)materialList.get(0);
				Object[] respList =  (Object[]) obj[4];
				int equipindex = 0;
				for (int i = 0; i < respList.length; ++i){
					if (oldResp.getEquipmentId().equalsIgnoreCase(((EquipmentDto)respList[i]).getEquipmentId()) == true){
						equipindex = i; materialCombo.setSelectedIndex(i);
					}
				}
			}
			...
			final JTextArea infoList = new JTextArea(oldcourse.getDescription());
			...
			jcc.setColor(oldcourse.getColor().intValue());
...
//System.out.println(cal.get(Calendar.DAY_OF_MONTH) + "/" + (cal.get(Calendar.MONTH)+1) + "/" + cal.get(Calendar.YEAR));
						...
						CourseDto newdto = new CourseDto(
							...
							//oldcourse.setOperation();
							oldcourse.setStartDate(startDate);
							oldcourse.setSubjectId(obj.getSubjectId());
							oldcourse.setSubjectType(typeCours);
							
							/*String id = */MainFrame.myDaybyday.updateCourse(oldcourse);//MainFrame.myDaybyday.createCourse(newdto);
